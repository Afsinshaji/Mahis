      ...
      if (isConnected) {
        if (global.appInfo.firebase != 'off') {
          ...
          await auth.signInWithCredential(_credential).then((result) async{
            log('Look at Me2 : ${result.toString()}');
            status = 'success';
            hideLoader();
...
//screenId ==0 -> Forgot Password
           ...
           await  _verifyViaFirebase(status);
            }
          }).catchError((e)async {
            status = 'failed';
            hideLoader();
 log('Look at Me3 : ${e.toString()}');
            if (screenId != null && screenId == 0) {
//screenId ==0 -> Forgot Password
              _firebaseOtpVerification(status);
            } else {
           await   _verifyViaFirebase(status);
            ...
            log('Look at Me4 : ${error.toString()}');
            log(error.toString());
            hideLoader();
          });
        } else {
...
// resend API
 ...
 Future _verifyViaFirebase(String _status) async {
    try {
      bool isConnected = await br.checkConnectivity();
      if (isConnected) {
        showOnlyLoaderDialog();
        await apiHelper.verifyViaFirebase(phoneNumber, _status, referalCode).then((result) async {
          if (result != null) {
            if (result.status == "1") {
              ...
              hideLoader();
              log('Look At me Now${result.message.toString()}');
              showSnackBar(key: _scaffoldKey, snackBarMessage: '${result.message}');
            }
          } else {
            hideLoader();
