...
class APIHelper {
  ...
  Future<dynamic> applyCoupon({String cartId, String couponCode}) async {
    try {
      ...
      dynamic recordList;
      if (response.statusCode == 200 && response.data["status"] == '1') {
        recordList = Orders.fromJson(response.data["data"]);
      } else {
        recordList = null;
      }
      return getDioResult(response, recordList);
...
// };
  ...
  log('k');
      //verifyOtpPassfirebase
      final url = Uri.parse('${global.baseUrl}checkout');
      ...
      final response = await http.post(url,
          body: body, headers: await global.getApiHeaders(true));
      ...
      log('0iii${response.body.toString()}');

      if (response.statusCode == 200) {
        log('0iii3');

        recordList = Orders.newOrder(
          cartIds: cartId,
      ...
      print("Exceptions - checkout(): " + e.toString());
    }
  }
  ...
  Future<dynamic> getActiveOrders(int page) async {
    try {
      ...
      dynamic recordList;
      if (response.statusCode == 200 && response.data["status"] == '1') {
        recordList = List<Orders>.from(
            response.data["data"].map((x) => Orders.fromJson(x)));
      } else {
        recordList = null;
      }
      return getDioResult(response, recordList);
  ...
  dynamic getDioResult<T>(final response, T recordList,
    ...
    try {
      ...
      result = DioResult.fromJson(response, recordList, isChanged: isChanged);
      return result;
    } catch (e) {
  ...
  Future<dynamic> verifyViaFirebase(
    ...
    try {
      log("look$phone$status$referralCode${global.appDeviceId}");
      ...
      //verifyOtpPassfirebase
      final url = Uri.parse('${global.baseUrl}verify_via_firebase');
      final body = jsonEncode({
        "user_phone": phone,
        "status": status,
        "referral_code": referralCode,
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonMap["status"].toString() == "1") {
          log('k');
          recordList = CurrentUser.fromJson(jsonMap["data"]);

          recordList.token = jsonMap["token"];
        } else {
          recordList = null;
        }
      } else {
      ...
      return getDioResult(response, recordList, isChanged: true);
    } catch (e) {
